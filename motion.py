...
def motion_detected(pixel1, pixel2, threshold, dtype=ANY_RGB):
    ...
    if ((dtype == ANY_RGB) and (r_diff > threshold) or (g_diff > threshold) or (b_diff > threshold)) or \
            ((dtype == ANY_2RGB) and (
                            (r_diff > threshold and g_diff > threshold) or (
                                        r_diff > threshold and b_diff > threshold) or (
                                    g_diff > threshold and b_diff > threshold))) or \
...
def compare_images(frame1, frame2, threshold=COMPUTE_THRESHOLD, dtype=KEY_REG):
    ...
    if frame1_size != frame2_size:
        print "compare_image :: Different image sizes"
        return False
    ...
    return float("{0:.2f}".format(detected_pixels * 100.0 / total_pixels))
